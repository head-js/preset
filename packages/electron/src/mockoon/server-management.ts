...
import { MockoonServer } from './server/server';
...
//   }
  ...
  private start() {
    const mainWindow = this.mainProcessBridge.getMainWindow();
...
//     });
    ...
    server.on('from-main.to-renderer', (payload) => {
      if (!mainWindow.isDestroyed()) {
